...
namespace Codeloader {
  ...
  cAllegro_IO::cAllegro_IO(std::string title, int width, int height, int scale, std::string font) {
    ...
    if (!allegro_ok) {
      throw Codeloader::cError("Could not initialize Allegro.");
    }
    bool keyboard_ok = al_install_keyboard();
    if (!keyboard_ok) {
      throw Codeloader::cError("Could not initialize keyboard.");
    }
    bool mouse_ok = al_install_mouse();
    if (!mouse_ok) {
      throw Codeloader::cError("Could not initialize mouse.");
    }
    bool image_ok = al_init_image_addon();
    if (!image_ok) {
      throw Codeloader::cError("Could not initialize image.");
    }
    bool font_ok = al_init_font_addon();
    if (!font_ok) {
      throw Codeloader::cError("Could not initialize font.");
    }
    bool ttf_ok = al_init_ttf_addon();
    if (!ttf_ok) {
      throw Codeloader::cError("Could not initialize the TTF font.");
    }
    bool audio_ok = al_install_audio();
    if (!audio_ok) {
      throw Codeloader::cError("Could not install audio.");
    }
    bool codec_ok = al_init_acodec_addon();
    if (!codec_ok) {
      throw Codeloader::cError("Could not install audio codec.");
    }
    ...
    if (!this->display) {
      throw Codeloader::cError("Could not create display.");
    }
    al_set_new_bitmap_flags(ALLEGRO_MEMORY_BITMAP);
    this->screen = al_create_bitmap(width, height);
    if (!this->screen) {
      throw Codeloader::cError("Could not create screen.");
    }
    this->canvas = al_create_bitmap(width, height);
    if (!this->canvas) {
      throw Codeloader::cError("Could not create canvas.");
    }
    this->icon = al_load_bitmap("Icon.png");
    if (!this->icon) {
      throw Codeloader::cError("Could not load window icon.");
    }
    al_set_display_icon(this->display, this->icon);
    this->font = al_load_font(std::string(font + ".ttf").c_str(), 14, 0);
    if (!this->font) {
      throw Codeloader::cError("Could not load font.");
    }
    al_install_joystick();
    this->timer = al_create_timer(1.0 / 20.0);
    if (!this->timer) {
      throw Codeloader::cError("Timer could not be created.");
    }
  ...
  void cAllegro_IO::Draw_Image(std::string name, int x, int y, int width, int height, int angle, bool flip_x, bool flip_y) {
    if (this->images.Does_Key_Exist(name)) {
      ...
      throw Codeloader::cError("Could not draw image " + name + ".");
    }
  }
  ...
  void cAllegro_IO::Play_Sound(std::string name) {
    if (this->sounds.Does_Key_Exist(name)) {
      ...
      throw Codeloader::cError("Could not find sound " + name + ".");
    }
  }
      ...
      while (!resource_file.eof()) {
        ...
        if (resource_file.good()) {
          ...
          if (pos == std::string::npos) {
            throw Codeloader::cError("File name not formatted correctly for " + file + ".");
          }
          ...
          if (ext == ".png") {
            ...
            if (image) {
              ...
              throw Codeloader::cError("Could not load image " + file + ".");
            }
          }
          else if (ext == ".wav") {
            ...
            if (sound) {
              ...
              throw Codeloader::cError("Could not load sound effect " + file + ".");
            }
          }
          else if (ext == ".ogg") {
            ...
            if (track.stream) {
              ...
              throw Codeloader::cError("Could not load sound track " + file + ".");
            }
          }
          else {
            throw Codeloader::cError("Invalid resource type " + ext + ".");
          }
        }
      }
    }
    else {
      throw Codeloader::cError("Could not load resources.");
    }
  }
  ...
  void cAllegro_IO::Load_Image(std::string name) {
    ...
    if (image) {
      ...
      throw Codeloader::cError("Could not load image " + name + ".");
    }
  }
      ...
      switch (event.type) {
        case ALLEGRO_EVENT_KEY_DOWN: {
          if (event.keyboard.keycode == ALLEGRO_KEY_ESCAPE) { // Program break key.
            throw Codeloader::cError("Program break!");
          }
  ...
  void cAllegro_IO::Play_Music(std::string name) {
    if (this->tracks.Does_Key_Exist(name)) {
      ...
      throw Codeloader::cError("Could not find track " + name + ".");
    }
  }
