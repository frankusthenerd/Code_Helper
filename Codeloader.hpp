// ============================================================================
// Codeloader Library (Implementation)
...
namespace Codeloader {
  ...
  int cConfig::Get_Property(std::string name) {
    if (!this->config.Does_Key_Exist(name)) {
      throw Codeloader::cError("Property value " + name + " does not exist.");
    }
  ...
  sSignal cIO_Control::Read_Key() {
    ...
    for (int signal_index = signal_count - 1; signal_count >= 0; signal_index--) { // Back to front.
